...
rf=RandomForestRegressor(n_estimators=1000,min_samples_leaf=8,max_features=10,min_samples_split=2)
rf.fit(x_train,y_train)
y_pred=rf.predict(x_test)

trainscore=rf.score(x_train,y_train)
valscore=rf.score(x_test,y_test)
#y_pred=rf.predict(x_test)
...
print ('R2 VALUE>>',rf.score(x_train, y_train), 'ADJUSTED R2 value>>',1 - (1-rf.score(x_train, y_train))*(len(y_train)-1)/(len(y_train)-x_train.shape[1]-1))
...
#export to csv file
testpredictions=rf.predict(X)
final_solution=pd.DataFrame(testpredictions,columns=['price'])
final_solution.to_excel('RandomFOrestRegressor.xlsx')
